...
#ifdef CONFIG_SH_AUDIO_DRIVER
	s32 rx_vol_step[VOC_RX_VOL_ARRAY_NUM][VOC_RX_VOL_STEP_NUM];
#else
			...
			void *private_data)
{
	...
	case AUDDEV_EVT_DEV_RDY:
		...
		if (evt_payload->voc_devinfo.dev_type == DIR_RX) {
			...
			for (i = 0; i < VOC_RX_VOL_ARRAY_NUM; i++) {
				memcpy(v->rx_vol_step[i], evt_payload->voc_devinfo.rx_vol_step[i],sizeof(v->rx_vol_step[i]));
			}
#else
			for (i = 0; i < VOC_RX_VOL_ARRAY_NUM; i++) {
				v->max_rx_vol[i] =
	...
	case AUDDEV_EVT_DEVICE_VOL_MUTE_CHG:
		if (evt_payload->voc_devinfo.dev_type == DIR_TX)
			...
			v->dev_rx.mute =
				evt_payload->voc_vm_info.rx_mute;
		}
#else
		else
			v->dev_rx.volume = evt_payload->
...
#ifdef CONFIG_SH_AUDIO_DRIVER
		v->dev_rx.mute = v->default_rx_mute_val;
#endif /* CONFIG_SH_AUDIO_DRIVER */
...
#ifdef CONFIG_SH_AUDIO_DRIVER
	step = ( (v->dev_rx.volume)*(VOC_RX_VOL_STEP_NUM-1) )/100;
	if (v->network == NETWORK_WCDMA_WB){
		vol = v->rx_vol_step[VOC_WB_INDEX][step];
	}
	else{
		vol = v->rx_vol_step[VOC_NB_INDEX][step];
	}
#else
	if (v->network == NETWORK_WCDMA_WB)
...
#ifdef CONFIG_SH_AUDIO_DRIVER
	for (i = 0; i < VOC_RX_VOL_ARRAY_NUM; i++) {
		memset(v->rx_vol_step[i], 0, sizeof(v->rx_vol_step[i]));
	}
#else
	for (i = 0; i < VOC_RX_VOL_ARRAY_NUM; i++) {
		v->max_rx_vol[i] = 0;
