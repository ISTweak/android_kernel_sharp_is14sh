...
#define OS_DESCRIPTOR_HEADER_SIZE	(2)

static u8 str_mtp_os_desc[] = {
		0x12,		/* length */
		USB_DT_STRING,		/* type */
		0x4d,0x00,0x53,0x00,0x46,0x00,	/* signature */
		0x54,0x00,0x31,0x00,0x30,0x00,0x30,0x00,
		USB_MS_OS_DESCRIPTOR_VENDER,	/* vendor code */
		0x00 		/* pad */
	};

#define USB_EXT_COMPAT_ID_INDEX_LENGTH		(0x00)
#define USB_EXT_COMPAT_ID_INDEX_IFNUM		(0x08)
#define USB_EXT_COMPAT_ID_SIZE_HEADER		(16)
#define USB_EXT_COMPAT_ID_SIZE_FUNCTION		(24)

static u8 str_mtp_ext_compat_id[] = {
		/* header size=16 */
		0x58,0x00,0x00,0x00,	/* length */
		0x00,0x01,		/* version */
		0x04,0x00,		/* index */
		0x03,			/* count */
		0x00,0x00,0x00,0x00,	/* reserve(7) */
		0x00,0x00,0x00,
		/* function size=24 */
		0x00,			/* interface */
		0x01,			/* reserve(1) */
		0x4D,0x54,0x50,0x00,	/* compatibleID(8) */
		0x00,0x00,0x00,0x00,
		0x00,0x00,0x00,0x00,	/* subCompatible(8) */
		0x00,0x00,0x00,0x00,
		0x00,0x00,0x00,0x00,	/* reserve(6) */
		0x00,0x00,

		/* function size=24 */
		0x01,			/* interface */
		0x01,			/* reserve(1) */
		0x00,0x00,0x00,0x00,	/* compatibleID(8) */
		0x00,0x00,0x00,0x00,
		0x00,0x00,0x00,0x00,	/* subCompatible(8) */
		0x00,0x00,0x00,0x00,
		0x00,0x00,0x00,0x00,	/* reserve(6) */
...
#ifdef CONFIG_USB_ANDROID_SH_MTP
static int setup_os_descriptor(struct usb_configuration *config)
{
	...
	if (list_empty(&config->functions))
		goto done;
	...
	res_len = USB_EXT_COMPAT_ID_SIZE_HEADER + 
				count * USB_EXT_COMPAT_ID_SIZE_FUNCTION;

	str_mtp_ext_compat_id[USB_EXT_COMPAT_ID_INDEX_LENGTH] = res_len;
	str_mtp_ext_compat_id[USB_EXT_COMPAT_ID_INDEX_IFNUM] = count;
	value = res_len;
done:
	return value;
}
		...
		enum usb_device_speed speed, void *buf, u8 type)
{
	...
	list_for_each_entry(f, &config->functions, list) {
		...
		if (f->disabled || !descriptors || descriptors[0] == NULL)
			continue;
#ifdef CONFIG_USB_ANDROID_SH_CUST
		if (f->reset_descriptor)
			f->reset_descriptor(config, f, descriptors, interfaceCount);
#endif /* CONFIG_USB_ANDROID_SH_CUST */
		status = usb_descriptor_fillbuf(next, len,
...
#ifdef CONFIG_USB_ANDROID_SH_MTP
	if (id == USB_MS_OS_DESCRIPTOR_ID) {
		struct usb_string_descriptor	*s = buf;
		u8 need = 0;
		struct usb_configuration	*c;

		/* we treat only one config setting */
		c = list_first_entry(&cdev->configs, 
				struct usb_configuration ,list);

		if (!c) {
			DBG(cdev, "get_string :OS_DESC no config\n");
			return -EINVAL;
		}
		...
		list_for_each_entry(f, &c->functions, list) {
			...
			if (!f) {
				DBG(cdev, "get_string :OS_DESC no function\n");
				break;
			}
			if (f->disabled)
				continue;
		...
		s->bDescriptorType = USB_DT_STRING;
		s->bLength = sizeof(str_mtp_os_desc);
		memcpy(s->wData,
				str_mtp_os_desc + OS_DESCRIPTOR_HEADER_SIZE,
				s->bLength - OS_DESCRIPTOR_HEADER_SIZE);
		return s->bLength;
	}
#endif /* CONFIG_USB_ANDROID_SH_MTP */
...
#ifdef CONFIG_USB_ANDROID_SH_CUST
		intf_cnt = 0;
		for (intf_id = 0; intf_id < MAX_CONFIG_INTERFACES; intf_id++) {
			f = cdev->config->interface[intf_id];
			if (!f || f->disabled)
				continue;
			if (w_index == intf_cnt++)
				break;
		}
		if (intf_id == MAX_CONFIG_INTERFACES)
			break;
#else /* CONFIG_USB_ANDROID_SH_CUST */
...
unknown:
...
#ifdef CONFIG_USB_ANDROID_SH_MTP
		if (value == D_USB_GADGET_SETUP_PENDING)
			goto done;
#endif /* CONFIG_USB_ANDROID_SH_MTP */
		...
		if ((ctrl->bRequestType & USB_TYPE_MASK)
				== USB_TYPE_VENDOR) {
			if (ctrl->bRequest == USB_MS_OS_DESCRIPTOR_VENDER) {
				u8 res_len = 0;
				struct usb_configuration	*c;
				...
				res_len = setup_os_descriptor(c);
				if (res_len < 0)
					goto done;

				value = min(w_length, (u16) res_len);
				memcpy(req->buf, str_mtp_ext_compat_id, value);
